...
MODEL_NAME = "HuggingFaceTB/SmolLM2-135M-Instruct"  # the small 135 million parameter model being used
...
tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)  # loads the tokenizer: converts text to numbers the model understands
...
def generate_question(context):
    prompt = (
        ...
        "Question:"  # the model writes from here, producing the question
    )
    ...
    with torch.no_grad():  # disables gradient tracking since we are not training, just generating
        outputs = model.generate(
            **inputs,
            ...
            temperature=0.7,  # adds some randomness so questions arent always identical
    ...
    question = re.sub(r"^question\s*:\s*", "", question, flags=re.IGNORECASE).strip()  # removes any accidental "question:" prefix
...
if __name__ == "__main__":
    ...
    app.run(debug=False, host="127.0.0.1", port=5000)  # starts the web server on port 5000